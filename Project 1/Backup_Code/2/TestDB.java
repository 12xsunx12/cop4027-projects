...
public class TestDB {
   ...
   public static void main(String[] args) throws Exception {  
      ...
      // Query the data with prepared statements
	  ...
	  pStatement = conn.prepareStatement("SELECT * FROM Test2 WHERE (make = ?) OR (make = ?)");
	  pStatement.setString(1, "Chevy");
	  pStatement.setString(2, "Toyota");
	  System.out.println(dbc.Query("SELECT * FROM Test2 WHERE (make = ?) OR (make = ?)", pStatement.executeQuery()));
      
      System.out.println("~~ Result Set 3 - All vehicles that weigh more than 2500lbs ~~");
      pStatement = conn.prepareStatement("SELECT * FROM Test2 WHERE weight >= ?");
      pStatement.setFloat(1, 2500);
      System.out.println(dbc.Query("SELECT * FROM Test2 WHERE weight >= ?", pStatement.executeQuery()));
      
      // Display the log file
      System.out.println("~~ SQL Command Log ~~\n" + dbc.PrintLogFile());
